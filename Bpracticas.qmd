...
## 🔐 Ética y responsabilidad en bioinformática
...
## ⭐ Puntos clave para buenas prácticas en bioinformática

### 
✏️ 1. Escribe programas para personas, no para máquinas
...
### 
🚩 4. Planifica los errores (**programación defensiva)**
...
-   ~Es\ una\ práctica\ donde\ **asumes\ que\ pueden\ ocurrir\ errores**\ y\ **preparas\ tu\ código**\ para\ detectarlos,\ avisar\ y\ manejar\ la\ situación\ sin\ que\ todo\ truene.\
    ...
    `if [ ! -s "$file" ]; then echo "Error: archivo vacío o no existe" >&2 exit 1 fi`

    `r`

    `if (!file.exists(fasta_path)) {    stop("El archivo FASTA no existe: ", fasta_path) }`

    `seqs <- read.fasta(fasta_path) if (length(seqs) == 0) { stop("El archivo FASTA está vacío.") }`
    :::

### 
⚙️ 5. Optimiza al final
...
## 📁 1. Organiza tus proyectos con *project-based workflows*
...
## 📦 2. Carga solo los paquetes necesarios

-   Evita cargar 10 paquetes si solo usas 1 función.

-   Usa **`pacman::p_load()`** o declara explícitamente tus dependencias.

-   Incluye siempre una sección “Paquetes” en tus scripts `.R`.

## ✏️ 3. Fórmula de oro: *estilo + consistencia*
...
## 
🔄 5. Haz tu código *pipeline-friendly*
...
## 🧪 6. Valida tus datos antes de analizarlos
...
## 🌱 7. Haz que tus gráficos sean reproducibles
...
## 🧬 9. Usa herramientas de análisis reproducible
...
## 🧼 10. Limpia tu entorno (pero no demasiado)
...
::: callout-note
## Ejemplo de como Evelia Coss realiza sus documentos 💜

Aqui les dejamos el script que le da a sus alumnos [VisualizacionDatos.R](https://github.com/EveliaCoss/RNAseq_classFEB2024/blob/main/Practica_Dia3/scripts/VisualizacionDatos.R) del curso de [Análisis de datos de RNA-Seq](https://github.com/EveliaCoss/RNAseq_classFEB2024).
:::
